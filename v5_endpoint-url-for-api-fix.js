// ==UserScript==
// @name         Suno API Bypasser & Unlocker (V6.3)
// @namespace    http://tampermonkey.net/
// @version      6.3
// @description  Bypasses React validation via direct API execution against Suno's Production Studio API.
// @author       User
// @match        *://*.suno.com/*
// @match        *://*.suno.ai/*
// @grant        none
...
(function() {
    ...
    async function forceApiCreate(clipId, type = "vox") {
        ...
        try {
            // HARDCODED TO THE PRODUCTION BACKEND YOU DISCOVERED
            const targetUrl = "https://studio-api.prod.suno.com/api/persona/create/";
            ...
            try {
                ...
                alert(`❌ ENDPOINT ERROR!\n\nThe server returned an HTML webpage instead of JSON.\nURL: ${targetUrl}\nResponse:\n${text.substring(0, 150)}...`);
                return;
            }
            ...
            if (res.ok) {
                ...
                alert(`❌ BACKEND REJECTED!\n\nThe server database blocked this.\nStatus: ${res.status}\nDetail: ${JSON.stringify(data)}`);
            }
        } catch (err) {
    ...
    function patchElement(el) {
        // ANTI-LADDER PROTECTION: Check if this exact element has been patched.
        if (el.dataset.apiBtnsInjected === "true") return;
        
        // ANTI-LADDER PROTECTION 2: Check if the parent menu already contains our custom UI.
        if (el.parentNode && el.parentNode.querySelector('.suno-api-dash')) {
        ...
        if (clipId) {
            ...
            dashboard.className = "suno-api-dash"; 
            ...
            if (el.parentNode) {
                el.parentNode.insertBefore(dashboard, el.nextSibling);
            }
        }
    }
    ...
    console.log("🚀[Suno Unlocker V6.3] Targeted Production Studio API Endpoint.");
})();
